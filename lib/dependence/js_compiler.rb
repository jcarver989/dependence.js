...
class JsCompiler
  ...
  def compile(output_file)
    ...
    puts "Source Files: #{file_order.to_s}"
  ...
  def get_source_files
    Dir.glob(@source_path + "/**/*.js")
  end

  def execute_compile(output_file)
    js_string = `#{@cmd}`
    File.open(output_file,'w') { |f| f.syswrite(js_string) }
    puts Colors.green("merged #{@source_files.size} files into #{output_file}")
    puts "------------------------------------------------------"
  end
